...
directory = "/Users/ecauet/Documents/METASPACE/Fukui-Model/Compounds-Alex/Compound2/Forces/"
Bondlimit = 2.8
#Factor = 0.52918
...
def project(N, list_atom, list_coord, list_gradient, list_bonds):
    ...
    for j in range(0, len(list_bonds), 1):
            ...
            ProjX1=ScalarProj1*(BondX/Bond)
            ProjY1=ScalarProj1*(BondY/Bond)
            ProjZ1=ScalarProj1*(BondZ/Bond)
                        
            ScalarProj2=((list_gradient[list_indexat2[j]][0]*(-BondX))+(list_gradient[list_indexat2[j]][1]*(-BondY))+(list_gradient[list_indexat2[j]][2]*(-BondZ)))/Bond
            theta2=(math.acos(ScalarProj2/list_force[list_indexat2[j]]))*180.0/math.pi

            ProjX2=ScalarProj2*((-BondX)/Bond)
            ProjY2=ScalarProj2*((-BondY)/Bond)
            ProjZ2=ScalarProj2*((-BondZ)/Bond)

            Fb=math.sqrt(math.pow(list_force[list_indexat1[j]],2)+math.pow(list_force[list_indexat2[j]],2))
            ...
            tmplst1=[]
            tmplst1 = [list_atom[list_indexat1[j]][0], list_coord[list_indexat1[j]][0], list_coord[list_indexat1[j]][1], list_coord[list_indexat1[j]][2], '{:.5f}'.format(ProjX1), '{:.5f}'.format(ProjY1), '{:.5f}'.format(ProjZ1)]
            tmplst1='   '.join([str(item) for item in tmplst1])                                       
            list_final.append(tmplst1)
            tmplst2=[]
            tmplst2 = [list_atom[list_indexat2[j]][0], list_coord[list_indexat2[j]][0], list_coord[list_indexat2[j]][1], list_coord[list_indexat2[j]][2], '{:.5f}'.format(ProjX2), '{:.5f}'.format(ProjY2), '{:.5f}'.format(ProjZ2)]
            tmplst2='   '.join([str(item) for item in tmplst2])                                       
            list_final.append(tmplst2)                                          


            tmplst1bis=[]                    
            tmplst1bis = [list_indexat1[j], list_indexat2[j], list_atom[list_indexat1[j]][0],list_atom[list_indexat2[j]][0], list_coord[list_indexat1[j]][0], list_coord[list_indexat1[j]][1], list_coord[list_indexat1[j]][2], list_force[list_indexat1[j]], ScalarProj1, theta1, '{:.5f}'.format(ProjX1), '{:.5f}'.format(ProjY1), '{:.5f}'.format(ProjZ1)]
            if  theta1 >= 90:
                tmplst1bis += ['Bond ELONGATION']
            list_final2.append(tmplst1bis)
            tmplst2bis=[]                    
            tmplst2bis = [list_indexat2[j], list_indexat1[j], list_atom[list_indexat2[j]][0],list_atom[list_indexat1[j]][0], list_coord[list_indexat2[j]][0], list_coord[list_indexat2[j]][1], list_coord[list_indexat2[j]][2], list_force[list_indexat2[j]], ScalarProj2, theta2, '{:.5f}'.format(ProjX2), '{:.5f}'.format(ProjY2), '{:.5f}'.format(ProjZ2)]
            if  theta2 >= 90:
                tmplst2bis += ['Bond ELONGATION']
            list_final2.append(tmplst2bis)


            tmplst1tris=[]
            tmplst1tris = [list_indexat1[j], list_indexat2[j],list_atom[list_indexat1[j]][0],list_atom[list_indexat2[j]][0],ProjX1,ProjY1,ProjZ1]
            list_proj.append(tmplst1tris)
            tmplst2tris=[]
            tmplst2tris = [list_indexat2[j], list_indexat1[j],list_atom[list_indexat2[j]][0],list_atom[list_indexat1[j]][0],ProjX2,ProjY2,ProjZ2]
            list_proj.append(tmplst2tris)

            
            tmplst3=[]
            tmplst3 = [list_indexat1[j], list_indexat2[j], Fb]
            list_forcebonds.append(tmplst3)
            
            
            tmplst4=[]
            tmplst4 = [list_indexat1[j], list_indexat2[j], Fpb]
            list_projforcebonds.append(tmplst4)
            
    j=0
    

    return list_indexat1, list_indexat2, list_force, list_final, list_final2, list_proj, list_forcebonds, list_projforcebonds
...
"""
myfolder = directory+"GRADIENT_NA/"
...
for f in os.listdir(myfolder):
    if f.endswith("NA.inp"):
        ...
        outputAN_file = open(os.path.join(myfolder+"output_"+str(f)[-7:-4]), "w")
        scalar_file = open(os.path.join(myfolder+"scalar_"+str(f)[-7:-4]), "w")
        proj_file = open(os.path.join(myfolder+"proj_"+str(f)[-7:-4]), "w")
        vector_file = open(os.path.join(myfolder+"vector_"+str(f)[-7:-4]), "w")
        frag_file = open(os.path.join(myfolder+"frag_"+str(f)[-7:-4]), "w")
        outfrag_force = open(os.path.join(myfolder+"frag_outF_"+str(f)[-7:-4]), "w")
        outfrag_proj = open(os.path.join(myfolder+"frag_outP_"+str(f)[-7:-4]), "w")
        ...
        print 'listAN_coord: ', listAN_coord
        print 'AN_atomsXYW: ', AN_atomsXYZ
        ...
        listAN_indexat1, listAN_indexat2, listAN_force, listAN_final, listAN_final2, listAN_proj, listAN_forcebonds, listAN_projforcebonds = project(NtotatomAN, listAN_atom, listAN_coord, listAN_gradient, AN_bonds)
        ...
        listAN_typebonds = []
        listAN_typebonds = [1] * len(AN_bonds)            
