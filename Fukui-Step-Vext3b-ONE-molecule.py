...
directory = "/Volumes/SAMSUNG-Stu-Emi/EMILIE/Postdoc_Imperial2/METASPACE/Fukui-Model/Compounds-Alex/Compound1/Forces/"
Bondlimit = 3.2
#Factor = 0.52918
...
"""
FUNCTION 5:
TO EVALUATE THE FORCES PER BOND
"""
def bondforce(num_proj, list_proj):
    ...
    for i in range(0, num_proj, 1):
        for j in range(0, num_proj, 1):
            if list_proj[i][0] == list_proj[j][1] and list_proj[i][1] == list_proj[j][0]:
                ...
                FbX=list_proj[i][4]+list_proj[j][4]
                FbY=list_proj[i][5]+list_proj[j][5]
                FbZ=list_proj[i][6]+list_proj[j][6]
                Fb=math.sqrt(math.pow(FbX,2)+math.pow(FbY,2)+math.pow(FbZ,2))
                
                temp=[]
                temp = [list_proj[i][0], list_proj[i][1], Fb]
...
for f in os.listdir(myfolder):
    if f.endswith("NA.inp"):
        ...
        output_frag = open(os.path.join(myfolder+"output_frag_"+str(f)[-7:-4]), "w")
...
#       nproj, listAN_final, listAN_final2, outputAN_file = project(NtotatomAN, Bondlimit, listAN_atom, listAN_coord, listAN_gradient)
        ...
        ### Evaluate the forces per bond - Function BondForce ####
        listAN_forcebonds = bondforce(nproj, listAN_proj)
...
#        AN_forces=listAN_force
        ...
        print>>output_frag, (frag1.Fragments), "\n"
