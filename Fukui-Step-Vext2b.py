...
"""
FUNCTION 0:
TO GENERATE A LIST WITH THE WEIGHT / ISOTOPE / CHARGE / ATOMATICITY OF EACH ATOM
"""
        ...
        for j in range(0, N, 1):
            if j !=i:
                ...
                if Bond <= limit:
                    ...
                    tmplst=[]
                    tmplst = [list_atom[i][0], list_coord[i][0], list_coord[i][1], list_coord[i][2], '{:.10f}'.format(ProjX), '{:.10f}'.format(ProjY), '{:.10f}'.format(ProjZ)]
...
#                    list_final += [round(ProjX, 3), round(ProjY, 3), round(ProjZ, 3)]
                    ...
                    tmplst2=[]                    
                    tmplst2 = [list_atom[i][0],list_atom[j][0], i, j, list_coord[i][0], list_coord[i][1], list_coord[i][2], Bond, Force, ScalarProj, theta, '{:.10f}'.format(ProjX), '{:.10f}'.format(ProjY), '{:.10f}'.format(ProjZ)]
...
#    return num_proj, list_final, list_final2, output_file
    return num_proj, list_force, list_bonds, list_final, list_final2
...
for FP in FPID:
    ...
    for f in os.listdir(myfolder):
        if f.endswith("A.inp"):
            ...
            final_frag = open(os.path.join(myfolder+"final_frag_"+str(f)[-7:-4]), "w")
...
#           nproj, listAN_final, listAN_final2, outputAN_file = project(NtotatomAN, Bondlimit, listAN_atom, listAN_coord, listAN_gradient)
            nproj, listAN_force, listAN_bonds, listAN_final, listAN_final2 = project(NtotatomAN, Bondlimit, listAN_atom, listAN_coord, listAN_gradient)
            ...
            for k in range(0, NtotatomAN, 1):
                tmplst4=[]
                tmplst4 = [listAN_atom[k][0], listAN_coord[k][0], listAN_coord[k][1], listAN_coord[k][2], '{:.10f}'.format(listAN_gradient[k][0]), '{:.10f}'.format(listAN_gradient[k][1]), '{:.10f}'.format(listAN_gradient[k][2])]
...
#            print>>frag_file, listAN_weight, "\n"
            ...
            AN_bonds=listAN_final6
            AN_atomsXYZ=listAN_final7
            AN_newbonds=fg.GetUniqueBonds(AN_bonds)
            print (AN_newbonds)
            
            frag1 = fg.FragmenterBasic(AN_atoms, AN_forces, AN_newbonds, AN_atomsXYZ, breakage_threshold=0.5, breakage_threshold_absolute=False, fragmentation_depth=2, Group_Atom_Numbers=True)
            frag1.SortResults(Ascending=False);
            print(frag1.Fragments)
            print>>final_frag, (frag1.Fragments), "\n"
